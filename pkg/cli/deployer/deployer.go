...
func (d *Deployer) Deploy(namespace, rcName string) error {
	...
	for _, candidate := range deployments {
		...
		if err := d.scaler.Scale(candidate.Namespace, candidate.Name, uint(0), nil, &kscale.ScalePrecondition{Size: -1, ResourceVersion: ""}, retryWaitParams, retryWaitParams, corev1.SchemeGroupVersion.WithResource("replicationcontrollers"), false); err != nil {
