...
func (r *RollingUpdater) scaleAndWaitWithScaler(rc *corev1.ReplicationController, retry, wait *kscale.RetryParams) (*corev1.ReplicationController, error) {
	...
	if err := scaler.Scale(rc.Namespace, rc.Name, uint(*rc.Spec.Replicas), nil, &kscale.ScalePrecondition{Size: -1}, retry, wait, corev1.SchemeGroupVersion.WithResource("replicationcontrollers"), false); err != nil {
		return nil, err
	}
